...
const AUDIT_LINES: Line[] = [
  { text: "$ npx agentguard audit", tone: "muted" },
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <div className="flex items-center gap-2 border-b border-border bg-secondary px-4 py-2.5">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-muted-foreground/40" aria-hidden="true" />
        <span className="h-2.5 w-2.5 rounded-full bg-muted-foreground/40" aria-hidden="true" />
        <span className="ml-2 font-mono text-xs text-muted-foreground">agentguard — audit</span>
      </div>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-6 max-w-xl text-pretty leading-relaxed text-muted-foreground">
              AgentGuard is npm audit + lockfile + permission firewall for the skills, MCP servers, and plugins you
