...
// This file assembles the dashboard view (the DashArtifact wire shapes defined
// in dto.go) from the live inventory joined with the locked snapshot: BuildScan
// and AttachLineDiffs plus the pure mappers that fold domain types into the DTO.
// It is IO-free — all scanning, lockfile reads, and signature checks happen in
